...
class ForecastFormPage extends LitElement {
  ...
  async _loadDropdownData() {
    try {
      ...
      // Load customer-type business partners for selection
      this.partners = await ds.getBusinessPartners({ partnerType: 'customer' });
      // Provide a customer labels array for backward display if needed
      this.customers = this.partners.map(p => `${p.accountNumber || ''}${p.accountNumber ? ' • ' : ''}${p.name}`);
    } catch (e) {
      this.partners = [];
      this.customers = [];
    }
                  ...
                  <div class="mb-4">
                    ...
                    <select
                      ...
                      @change=${(e) => {
                        ...
                        const p = this.partners.find(x => x.id === selectedId);
                        this._updateField('counterpartyId', selectedId);
                    ...
                    >
                      ...
                      ${this.partners.map(p => html`
                        <option value="${p.id}" ?selected=${this.formData.counterpartyId === p.id}>
                          ${p.accountNumber || ''}${p.accountNumber ? ' • ' : ''}${p.name}
                        </option>
                      `)}
                    </select
                  </div>
...
customElements.define('inbox-invoice-form', ForecastFormPage);
