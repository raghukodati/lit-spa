...
class ForecastListPage extends LitElement {
  ...
  _displayCustomer(row) {
    if (row.customerId) {
      const label = getBusinessPartnerLabelById(row.customerId);
      return label || row.customerName || '';
    }
                ...
                <table class="table table-hover table-striped mb-0">
                  ...
                  <tbody>
                    ${this.forecasts.length > 0 ? this.forecasts.map(forecast => html`
                      <tr>
                        ${this.visibleColumns.customerName ? html`<td><strong>${this._displayCustomer(forecast)}</strong></td>` : ''}
...
customElements.define('outbox-purchase-orders-list', ForecastListPage);
