/**
 * Payment Methods Component - Manage saved payment methods
 */
...
class PaymentMethods extends LitElement {
  ...
  loadPaymentMethods() {
    try {
      const stored = localStorage.getItem('customer-payment-methods');
      return stored ? JSON.parse(stored) : this.getDefaultPaymentMethods();
    } catch (e) {
  ...
  savePaymentMethods() {
    localStorage.setItem('customer-payment-methods', JSON.stringify(this.paymentMethods));
  }
  ...
  handleFormSubmit(e) {
    ...
    if (type === 'credit_card') {
      ...
      methodData = {
        ...
        lastFour: cardNumber ? cardNumber.slice(-4) : this.editingMethod.lastFour,
      ...
      methodData = {
        ...
        lastFour: accountNumber ? accountNumber.slice(-4) : this.editingMethod.lastFour,
    ...
    if (this.isNew) {
      ...
      this.paymentMethods = this.paymentMethods.map(method =>
        method.id === this.editingMethod.id ? { ...method, ...methodData } : method
      );
    }
    ...
    if (methodData.isDefault) {
      this.paymentMethods = this.paymentMethods.map(method => ({
        ...method,
        isDefault: method.id === (this.editingMethod?.id || Date.now())
      }));
    }
  ...
  renderPaymentCard(method) {
    return html`
      <div class="col-md-6 mb-3">
        <div class="card h-100 ${method.isDefault ? 'border-primary' : ''}">
          ...
          <div class="card-body">
            <h6 class="card-title">
              ${method.nickname}
              ${method.type === 'credit_card' ? html`
                <span class="badge bg-info ms-2">
                  <i class="bi bi-${this.getCardIcon(method.cardType)} me-1"></i>Card
                </span>
              ` : method.type === 'ach' ? html`
            ...
            ${method.type === 'credit_card' ? html`
              <div class="text-muted small">
                <p class="mb-1">
                  <i class="bi bi-${this.getCardIcon(method.cardType)} me-2"></i>
            ...
            ` : html`
              <div class="text-muted small">
                ...
                <p class="mb-1">
                  ...
                  Credit Limit: ${this.formatCurrency(method.creditLimit)}
                </p>
                <p class="mb-0 text-success">
                  <i class="bi bi-check-circle me-2"></i>
                  Available: ${this.formatCurrency(method.availableCredit)}
                </p>
              </div>
            `}
          </div>
            ...
            <div class="d-flex gap-2">
              ${!method.isDefault ? html`
                <button class="btn btn-sm btn-outline-primary" 
                        @click=${() => this.setDefaultMethod(method.id)}
                        title="Set as default">
                  <i class="bi bi-star"></i> Set Default
                </button>
              ` : ''}
              ${method.type !== 'net_terms' ? html`
                <button class="btn btn-sm btn-outline-secondary" 
                        @click=${() => this.editMethod(method)}>
                  <i class="bi bi-pencil"></i> Edit
                </button>
                <button class="btn btn-sm btn-outline-danger" 
                        @click=${() => this.deleteMethod(method.id)}>
      ...
      <div class="container-fluid mt-4">
        ...
        <div class="row">
          ${this.paymentMethods.length > 0 ? html`
            ${this.paymentMethods.map(method => this.renderPaymentCard(method))}
          ` : html`
            <div class="col-12">
              ...
              <div class="modal-content">
                <form @submit=${this.handleFormSubmit}>
                  ...
                  <div class="modal-body">
                    ...
                    ${this.editingMethod?.type === 'credit_card' ? html`
                      <div class="row">
                        <div class="col-md-6 mb-3">
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                          ...
                          <select class="form-select" name="expiryYear" .value=${this.editingMethod?.expiryYear || ''} required>
                            ...
                            ${Array.from({length: 10}, (_, i) => new Date().getFullYear() + i).map(y => html`
