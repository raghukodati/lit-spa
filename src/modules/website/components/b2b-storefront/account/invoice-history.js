/**
 * Invoice History Component - View and manage invoices
 */

import { LitElement, html } from 'lit';

class InvoiceHistory extends LitElement {
  static properties = {
    invoices: { type: Array },
    ...
    return [
      {
        ...
        balance: 0,
        items: [
          { name: 'Professional Laptop', quantity: 2, price: 1299.99 },
          { name: 'Wireless Mouse', quantity: 5, price: 29.99 }
        ],
        customerPO: 'CUST-PO-789',
        paymentTerms: 'NET30',
        notes: 'Paid via ACH'
      },
      {
        ...
        balance: 5678.25,
        items: [
          { name: 'Office Desk', quantity: 3, price: 849.99 },
          { name: 'Office Chair', quantity: 3, price: 599.99 }
        ],
        customerPO: 'CUST-PO-790',
        paymentTerms: 'NET30',
        notes: ''
      },
      {
        ...
        balance: 1450.00,
        items: [
          { name: 'Monitor 27"', quantity: 4, price: 449.99 },
          { name: 'HDMI Cable', quantity: 4, price: 19.99 }
        ],
        customerPO: 'CUST-PO-791',
        paymentTerms: 'NET30',
      ...
      },
      {
        ...
        balance: 8999.99,
        items: [
          { name: 'Heavy Equipment', quantity: 1, price: 8999.99 }
        ],
        customerPO: 'CUST-PO-788',
        paymentTerms: 'NET30',
        notes: 'Payment overdue - please contact billing'
      }
    ];
  }
  ...
  payInvoice(invoice) {
    window.location.pathname = `/b2b-store/invoice/${invoice.id}/pay`;
  }

  get filteredInvoices() {
    return this.invoices.filter(invoice => {
      // Status filter
      if (this.statusFilter !== 'all' && invoice.status !== this.statusFilter) {
        return false;
      }
      ...
      if (this.searchTerm) {
        ...
        return invoice.invoiceNumber.toLowerCase().includes(searchLower) ||
               ...
               invoice.customerPO?.toLowerCase().includes(searchLower);
      }
  ...
  get totals() {
    return this.filteredInvoices.reduce((acc, inv) => ({
  ...
  render() {
    return html`
      <div class="container-fluid mt-4">
          ...
          <div class="card-body p-0">
            <div class="table-responsive">
              <table class="table table-hover mb-0">
                ...
                <tbody>
                  ${this.filteredInvoices.length > 0 ? html`
                    ${this.filteredInvoices.map(invoice => html`
                      <tr>
                        <td>
                          <strong>${invoice.invoiceNumber}</strong>
                          ${invoice.customerPO ? html`
                            <br><small class="text-muted">PO: ${invoice.customerPO}</small>
                          ` : ''}
                        </td>
                        <td>${this.formatDate(invoice.date)}</td>
                        <td>
                          ${this.formatDate(invoice.dueDate)}
                          ${invoice.status === 'overdue' ? html`
                        ...
                        <td>
                          <a href="/b2b-store/orders/${invoice.orderNumber}">${invoice.orderNumber}</a>
                        </td>
                        <td class="fw-bold">${this.formatCurrency(invoice.amount)}</td>
                        <td class="text-success">${this.formatCurrency(invoice.paidAmount)}</td>
                        <td class="${invoice.balance > 0 ? 'text-warning fw-bold' : 'text-muted'}">
                          ${this.formatCurrency(invoice.balance)}
                        </td>
                        <td>
                          <span class="badge ${this.getStatusBadge(invoice.status)}">
                            ${invoice.status}
                          </span>
                        </td>
                        <td>
                          <div class="btn-group btn-group-sm">
                            <button class="btn btn-outline-primary" 
                                    @click=${() => this.viewInvoice(invoice)}
                                    title="View Details">
                              <i class="bi bi-eye"></i>
                            </button>
                            <button class="btn btn-outline-secondary" 
                                    @click=${() => this.downloadInvoice(invoice)}
                                    title="Download PDF">
                              <i class="bi bi-download"></i>
                            </button>
                            ${invoice.balance > 0 ? html`
                              <button class="btn btn-outline-success" 
                                      @click=${() => this.payInvoice(invoice)}
                                      title="Pay Now">
        ...
        ${this.showDetailModal && this.selectedInvoice ? html`
            ...
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  ...
                  <div class="table-responsive mb-4">
                    <table class="table table-sm">
                      ...
                      <tbody>
                        ${this.selectedInvoice.items.map(item => html`
                          <tr>
                            <td>${item.name}</td>
                ...
                <div class="modal-footer">
                  ...
                  ${this.selectedInvoice.balance > 0 ? html`
...
customElements.define('invoice-history', InvoiceHistory);
