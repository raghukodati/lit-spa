/**
 * B2B Storefront Dashboard - Customer Portal
 */
...
class StorefrontDashboard extends LitElement {
  ...
  constructor() {
    ...
    this.recentOrders = [];
    this.loading = false;
    // Mock: Get current company from session/auth
    this.companyId = 1; // In real app, get from authentication
  }
  ...
  async loadData() {
    this.loading = true;
    try {
      [this.company, this.recentOrders] = await Promise.all([
        getCompanyById(this.companyId),
        getPurchaseOrders({ companyId: this.companyId, limit: 5 })
      ]);
    } catch (err) {
        ...
        <div class="row">
          ...
          <div class="col-lg-8">
            ...
            <div class="card mb-4">
              ...
              <div class="card-body">
                ${this.recentOrders && this.recentOrders.length > 0 ? html`
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ${this.recentOrders.slice(0, 5).map(order => html`
                          <tr style="cursor: pointer;" @click=${() => window.location.pathname = `/b2b-store/orders/${order.id}`}>
...
customElements.define('storefront-dashboard', StorefrontDashboard);
