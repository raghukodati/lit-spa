const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/dataService-DgEHmTaO.js","assets/index-DZz2m7sH.js","assets/company.service-BRgwWIrg.js","assets/company.model-CKmygYMs.js","assets/analytics.service-O2YxAu-9.js","assets/gdocs.service-BWkhXK5L.js","assets/incident.service-v2fXBIsd.js","assets/inbox.service-BbRVIr2l.js","assets/outbox.service-OkSsOYWd.js"])))=>i.map(i=>d[i]);
import{T as o,_ as r,N as s}from"./index-DZz2m7sH.js";import{getForecastYears as l,getForecastMonths as c,getForecastById as n,updateForecast as d,createForecast as m}from"./analytics.service-O2YxAu-9.js";class h extends o{static properties={forecastId:{type:String},formData:{type:Object},loading:{type:Boolean},saving:{type:Boolean},error:{type:String},isEditMode:{type:Boolean},customers:{type:Array},partners:{type:Array},years:{type:Array},months:{type:Array},calculatedCommission:{type:Number}};createRenderRoot(){return this}constructor(){super(),this.forecastId=null,this.formData={customerName:"",year:"2024",month:"January",forecastValue:0,commissionPercent:5},this.loading=!1,this.saving=!1,this.error="",this.isEditMode=!1,this.customers=[],this.partners=[],this.years=[],this.months=[],this.calculatedCommission=0}connectedCallback(){super.connectedCallback(),this._checkMode(),this._loadDropdownData()}_checkMode(){const t=window.location.pathname.match(/\/module\/analytics\/forecast\/edit\/(\d+)/);t?(this.isEditMode=!0,this.forecastId=t[1],this._loadForecast()):(this.isEditMode=!1,this._calculateCommission())}async _loadDropdownData(){try{const e=await r(()=>import("./dataService-DgEHmTaO.js"),__vite__mapDeps([0,1,2,3,4,5,6,7,8]));this.partners=await e.getBusinessPartners({partnerType:"customer"}),this.customers=this.partners.map(t=>`${t.accountNumber||""}${t.accountNumber?" • ":""}${t.name}`)}catch{this.partners=[],this.customers=[]}this.years=l(),this.months=c()}async _loadForecast(){this.loading=!0,this.error="";try{const e=await n(this.forecastId);e?(this.formData={customerName:e.customerName,year:e.year,month:e.month,forecastValue:e.forecastValue,commissionPercent:e.commissionPercent},this._calculateCommission()):this.error="Forecast not found"}catch(e){this.error=e.message||"Failed to load forecast"}finally{this.loading=!1}}async _handleSubmit(e){e.preventDefault(),this.saving=!0,this.error="";try{this.isEditMode?await d(parseInt(this.forecastId),this.formData):await m(this.formData),window.history.pushState({},"","/module/analytics/forecast"),window.dispatchEvent(new PopStateEvent("popstate"))}catch(t){this.error=t.message||"Failed to save forecast"}finally{this.saving=!1}}_handleCancel(){confirm("Are you sure you want to cancel? Unsaved changes will be lost.")&&(window.history.pushState({},"","/module/analytics/forecast"),window.dispatchEvent(new PopStateEvent("popstate")))}_updateField(e,t){this.formData={...this.formData,[e]:t},(e==="forecastValue"||e==="commissionPercent")&&this._calculateCommission()}_calculateCommission(){const e=parseFloat(this.formData.forecastValue)||0,t=parseFloat(this.formData.commissionPercent)||0;this.calculatedCommission=e*t/100}render(){return this.loading?s`
                ...
                <div class="card-body">
                  <div class="mb-4">
                    ...
                    <select
                      ...
                      .value=${this.formData.counterpartyId||""}
                      @change=${e=>{const t=e.target.value?parseInt(e.target.value):null,a=this.partners.find(i=>i.id===t);this._updateField("counterpartyId",t),this._updateField("customerName",a?`${a.accountNumber||""}${a.accountNumber?" • ":""}${a.name}`:"")}}
                      required
                      ?disabled=${this.saving}
                    >
                      <option value="">Select a customer...</option>
                      ${this.partners.map(e=>s`
                        <option value="${e.id}" ?selected=${this.formData.counterpartyId===e.id}>
                          ${e.accountNumber||""}${e.accountNumber?" • ":""}${e.name}
                        </option>
                      `)}
                    </select
                  </div>
    ...
    `}}customElements.define("outbox-invoice-form",h);
